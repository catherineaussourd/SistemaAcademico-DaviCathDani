...
public class removerTurmaController implements Initializable {
	...
	@FXML
	public void deletarTurma(ActionEvent event) {
		LinkedList<Turma> listaTurmas = leitorListas.ListaTurmas();
		...
		if (resultado != -1) {
			...
			ObservableList<Aluno> listaAlunos = tableViewTurma.getItems();
			if (cpfProfessor.getText() == "" || listaAlunos.isEmpty()) {

				try {
					listaTurmas.remove(resultado);
				...
				Alerts.showAlerts("Error", null, "Turma contem aluno ou professor, não é possivel excluir.", AlertType.ERROR);
			}
		}
