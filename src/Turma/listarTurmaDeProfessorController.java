...
public class listarTurmaDeProfessorController implements Initializable {
	...
	public void buscarProfessor(ActionEvent event) {
		LinkedList<Professor> listaProfessors = leitorListas.ListaProfessor();
		String cpfprocurar = cpfProf.getText();
		int resultado = buscar(cpfprocurar, listaProfessors);
		System.out.println("INDEX DO CPF: " + resultado);
		if (isCPF(cpfProf.getText()) == false) {
			Alerts.showAlerts("Error", null, "CPF Invalido!!", AlertType.ERROR);
		} else if (resultado != -1) {
			exibirLista();
	...
	public int buscar(String codigo, LinkedList<Professor> lista) {
		System.out.println("codigo PROCURADO:" + codigo);
		System.out.println("--------------");
		for (Professor prof : lista) {
			System.out.println("Comparar:" + prof.getCpf() + " e: " + codigo);
			if (prof.getCpf().compareToIgnoreCase(codigo) == 0) {

				return lista.indexOf(prof);
			}
		}
		return -1;
	}
	...
	public ObservableList<Turma> getTurmas() {
		...
		if (lista.isEmpty() != true) {
			for (Turma turma : lista) {
				try {
				if (turma.getCpfProfessor().equals(cpfProf.getText()) == true) {
